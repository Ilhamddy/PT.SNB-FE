...
mu.namaunit,to2.namaoperasi,mi.kodeexternal ||' '|| mi.reportdisplay as kodeexternal,
mp2.namalengkap,ms.reportdisplay as statusoperasi,
case when to2.iscito=true then '✓' else '✕' end as statuscito from t_orderoperasi to2 
...
const qDaftarPasienOperasi =`select 
    x.norecta,
    x.norecdp,
    x.objectkamarfk,
    x.catatanverif,
    x.objectjenisoperasifk,
    x.objectdokteroperatorfk,
    x.tglrencanax,
    x.objectstatusoperasifk,
    x.iscito,
    x.jenispenjamin,
    x.namakelas,
    x.pengorder,
    ...
    when x.days>23724  and x.objectjeniskelaminfk=2 then 'nenek' else 'baby' end as profile,
    x.tgllahir,
    x.objectjeniskelaminfk 
FROM (
    SELECT 
        to2.objectantreanpemeriksaanoperasifk as norecta,
        td.norec as norecdp,
        to2.objectkamarfk,
        to2.catatanverif,
        to2.objectjenisoperasifk,
        to2.objectdokteroperatorfk,
        to2.tglrencana as tglrencanax,
        to2.iscito,
        mj3.jenispenjamin,
        mk.namakelas,
        mp3.namalengkap as pengorder,
        to2.catatanorder,
        mj2.jeniskelamin,
        to2.objectunitasalfk,
        CASE WHEN to2.objectjenisoperasifk=1 
            THEN '#FFB2B2' 
            WHEN to2.objectjenisoperasifk=2 
            THEN '#FFE0B2'
            WHEN to2.objectjenisoperasifk=3 
            THEN '#B8FFB2' 
            ELSE '#5AEBFF' 
        END as colorjenisoperasi,
        mj.reportdisplay as jenisoperasi,
        mp.namapasien,td.noregistrasi,
        mp.nocm,to2.norec,
        to2.tglinput,
        to_char(to2.tglinput,'dd Month YYYY HH24:MI') as tglinputx,
        to_char(to2.tglrencana,'dd Month YYYY HH24:MI') as tglrencana,
        mu.namaunit,
        to2.namaoperasi,
        mi.kodeexternal,
        mp2.namalengkap,
        ms.reportdisplay as statusoperasi,
        to2.objectstatusoperasifk,
        (current_date - to_date(to_char(mp.tgllahir, 'DD-MM-YYYY'), 'DD-MM-YYYY')) AS days,
        to_char(mp.tgllahir, 'DD-MM-YYYY') as tgllahir,mp.objectjeniskelaminfk,
        DATE_PART('year',CURRENT_DATE) - DATE_PART('year', mp.tgllahir::date) || ' Tahun ' ||
        DATE_PART('month',CURRENT_DATE) - DATE_PART('month', mp.tgllahir::date) || ' Bulan' as umur 
    FROM t_orderoperasi to2 
        join t_antreanpemeriksaan ta on ta.norec=to2.objectantreanpemeriksaanoperasifk
        join t_daftarpasien td on td.norec=ta.objectdaftarpasienfk
        join m_pasien mp on mp.id=td.nocmfk
        join m_unit mu on mu.id=to2.objectunitasalfk
        join m_icdx mi on mi.id=to2.objecticdxfk
