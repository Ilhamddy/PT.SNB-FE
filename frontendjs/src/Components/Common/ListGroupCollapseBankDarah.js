...
const ListGroupCollapseBankDarah = ({ cat, tempData, index, onChange }) => {
    ...
    return (
        <Card className="card-animate">
            ...
            <Collapse isOpen={collapse}>
                <CustomCheckbox
                    data={stateDummy}
                    labelSelector={(data) => `${data.label} (${data.harga?.toLocaleString('id-ID')})`}
                    setData={(newData) => {
...
ListGroupCollapseBankDarah.propTypes = {
    tempData: PropTypes.any,
    show: PropTypes.any,
};

export default ListGroupCollapseBankDarah;