...
import { masterGetSuccess,
    ...
    getComboMappingProdukSuccess,
    getComboMappingProdukError,
    getComboViewerSuccess,
    getComboViewerError,
    getComboResepGlobalSuccess,
    getComboResepGlobalError
} from "./action";
...
function* onGetComboMappingProduk(){
    try {
        const response = yield call(serviceMaster.getComboMappingProduk);
        yield put(getComboMappingProdukSuccess(response.data));
    } catch(error){
        yield put(getComboMappingProdukError(error));
    }
}

function* onGetComboViewer() {
    try {
        const response = yield call(serviceMaster.getComboViewer);
        yield put(getComboViewerSuccess(response.data));
    } catch (error) {
        yield put(getComboViewerError(error));
    }
}

function* onGetComboResepGlobal() {
    try {
        const response = yield call(serviceMaster.getComboResepGlobal);
        yield put(getComboResepGlobalSuccess(response.data));
    } catch (error) {
        yield put(getComboResepGlobalError(error));
    }
}
...
export function* watchGetComboMappingProduk(){
    yield takeEvery(GET_COMBO_MAPPING_PRODUK, onGetComboMappingProduk);
}

export function* watchGetComboMaster(){
    yield takeEvery(GET_COMBO_VIEWER, onGetComboViewer);
}

export function* watchGetComboResepGlobal(){
    yield takeEvery(GET_COMBO_RESEP_GLOBAL, onGetComboResepGlobal);
}
...
function* masterSaga() {
    yield all([
        ...
        fork(watchGetComboMappingProduk),
        fork(watchGetComboMaster),
        fork(watchGetComboResepGlobal)
    ]);
}
