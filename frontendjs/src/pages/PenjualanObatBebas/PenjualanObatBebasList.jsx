...
import { getPenjualanBebas } from '../../store/farmasi/action'
...
const PenjualanObatBebasList = () => {
  const navigate = useNavigate()
  const dispatch = useDispatch()
  const { listOrder } = useSelector((state) => ({
    listOrder: state.Farmasi.getPenjualanBebas.data?.dataPenjualanBebas || [],
  }))
  ...
  const vFilter = useFormik({
    ...
    onSubmit: (values) => {
      dispatch(getPenjualanBebas(values))
    },
  })

  useEffect(() => {
    dispatch(getPenjualanBebas(vFilter.initialValues))
  ...
  const columnsOrder = [
          ...
          <UncontrolledDropdown className="dropdown d-inline-block">
            ...
            <DropdownMenu className="dropdown-menu-end">
              <DropdownItem
                onClick={() =>
                  navigate(
                    `/farmasi/penjualan-obat-bebas/${row.norecjualbebas}`
                  )
                }
              >
    ...
    {
      name: <span className="font-weight-bold fs-13">Penulis</span>,
      sortable: true,
      selector: (row) => row.namapenulis,
      width: '120px',
    },
    {
      ...
      <Container fluid>
        ...
        <Card className="p-5">
          <Row className="d-flex flex-row justify-content-between">
            <Col lg={9}>
              <Row>
                <ColLabelInput lg={4} label={'Tanggal Mulai'}>
                  <KontainerFlatpickr
                    isError={
                ...
                <ColLabelInput lg={4} label={'Tanggal Akhir'}>
                  <KontainerFlatpickr
                    isError={
            ...
            <ColLabelInput lg="auto">
              <Link to="/farmasi/penjualan-obat-bebas">
...
export default PenjualanObatBebasList
