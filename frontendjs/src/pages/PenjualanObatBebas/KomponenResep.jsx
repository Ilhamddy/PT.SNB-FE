...
export const useHandleChangeResep = (resepRef, vResep, estimasiKlaim, dataTagihan) => {
  ...
  const handleChangeObatResep = useCallback(
    (e, row) => {
      ...
      const hargaSatuan = e?.batchstokunit?.[0]?.harga || 0
      const [harga] = calculateRounding(
        (e?.batchstokunit?.[0]?.harga || 0) * margin
      )
      const nobatch = e?.batchstokunit?.[0]?.nobatch || ''
      // hitung harga
      let totalHarga = harga * (row.qty || 0) || 0
  ...
  const handleQtyObatResep = (e, row, val, setVal) => {
    ...
    let totalHarga = row.harga * (strToNumber(newVal) || 0)
    const [roundedHarga, difference] = calculateRounding(
      totalHarga,
      roundingTotal
    )
    ...
    row.racikan.forEach((valRacikan) => {
      ...
      let totalHargaRacikan = valRacikan.harga * strToNumber(qtyBulat) || 0
      const [roundedHarga, difference] = calculateRounding(
        totalHargaRacikan,
        roundingTotal
      )
  ...
  const handleQtyRacikan = useCallback(
    (e, row, rowUtama, val, setVal) => {
      ...
      let totalHarga = row.harga * strToNumber(qtyBulat) || 0
      const [roundedHarga, difference] = calculateRounding(
        totalHarga,
        roundingTotal
      )
  ...
  const handleChangeObatRacikan = (e, row, rowUtama) => {
    ...
    const totalHarga = harga * qtyTotal
    const [roundedHarga, difference] = calculateRounding(
      totalHarga,
      roundingTotal
    )
...
export const useColumnsResepRacikan = (
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [vResep.values.resep, obatList, allObat, disableObat, isQty, isAllObat])
