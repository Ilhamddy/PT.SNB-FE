...
const OrderBankDarah = () => {
  ...
  const { dataCombo, dataWidget, dataOrder, loadingOrder } = useSelector(
    (state) => ({
      dataCombo: state.Emr.comboHistoryUnitGet.data,
      dataWidget:
        state.bankDarahSlice.getDetailJenisProdukBankDarah?.data || [],
  ...
  const onClickSimpan = () => {
    ...
    dispatch(
      postOrderPelayananBankDarah(tempValue, () => {
        dispatch(getRiwayatOrderBankDarah({ norecdp: norecdp }))
      })
    )
  }
                  ...
                  <Col lg={8} md={8}>
                    <div>
                      <CustomSelect
                        ...
                        className={`input ${
                          validation.errors.unitasal ? 'is-invalid' : ''
                        }`}
                        onChange={(value) =>
                          validation.setFieldValue('unitasal', value?.value)
                        }
                      />
                      {validation.touched.unitasal &&
                      validation.errors.unitasal ? (
                        <FormFeedback type="invalid">
                          <div>{validation.errors.unitasal}</div>
                        </FormFeedback>
                      ) : null}
                    </div>
                  </Col>
